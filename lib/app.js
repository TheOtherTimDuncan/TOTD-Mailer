...
app.sendEmail = function (queueService, queueName, sg, sgmail, handlebars, messageText) {
    ...
    if (data.to.length > 1) { 
        for (var i = 1; i++; i < data.to.length) { 
            email.personalizations.addTo(data.to[i]);
        }
    }   
    ...
    email.setSubject(subjectTemplate(data.emailData));
    email.setHtml(htmlTemplate(data.emailData));
    email.setText( txtTemplate(data.emailData));
*/
/*    
    var email = new sendgrid.Email({
        to: data.to,
